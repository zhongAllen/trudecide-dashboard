...
# -*- coding: utf-8 -*-
"""
REQ-161: 上市公司高管信息采集脚本
====================================
从 Tushare Pro 的 `stk_managers` 接口采集上市公司高管任职信息，
...
更新策略：
  - UPSERT（ON CONFLICT ts_code, ann_date, name, title DO UPDATE）
...
# ── 常量 ──────────────────────────────────────────────────────────────────────
MODULE_NAME = "collect_stock_managers"
API_SLEEP   = 0.4    # 接口调用间隔（秒）
BATCH_SIZE  = 500    # 数据库写入批次大小

TS_FIELDS = "ts_code,ann_date,name,gender,lev,title,edu,national,birthday,begin_date,end_date,resume"
...
def upsert_batch(sb, rows: list) -> int:
    """
    分批 UPSERT 到 stock_managers 表。
    CONFLICT_COLS: ['ts_code', 'ann_date', 'name', 'title']
    （与数据库 UNIQUE INDEX idx_stock_managers_unique 一致）
    """
    total = 0
    for i in range(0, len(rows), BATCH_SIZE):
        batch = rows[i:i + BATCH_SIZE]
        sb.table("stock_managers").upsert(
            batch, on_conflict="ts_code,ann_date,name,title"
        ).execute()
        total += len(batch)
    return total
...
# ── 主采集流程 ─────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="上市公司高管信息采集脚本 (REQ-161)")
    ...
    print("=" * 60)
    print("REQ-161: stock_managers 采集开始")
    ...
    try:
        ...
        if args.dry_run:
            print("\n[DRY-RUN] 前 3 条数据预览：")
            for r in all_rows[:3]:
                print(f"  {r['ts_code']} | {r.get('name')} | {r.get('title')} | "
