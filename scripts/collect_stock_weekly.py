...
个股周线数据采集脚本（REQ-172）
===============================
用途：采集 A 股全量股票的周线行情，写入 stock_weekly 表
数据来源：Tushare Pro stk_weekly 接口
采集范围：stock_meta 中所有股票，最近 5 年
执行方式：
  python3 collect_stock_weekly.py [--mode full|incremental] [--dry-run] [--workers N]
  --mode full         全量采集（最近5年），默认
  --mode incremental  增量采集（仅最近 4 周）
  --dry-run           只打印，不写库
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('/tmp/stock_weekly_collect.log', mode='a'),
    ]
)
...
MAX_WORKERS   = 10      # 并发线程数（周K按股票切分，并发不宜过高）
BATCH_SIZE    = 500     # upsert 批次大小
API_SLEEP     = 0.3     # 每次请求间隔（秒）- 周K接口较慢，间隔加大
RETRY_TIMES   = 3       # 失败重试次数
RETRY_SLEEP   = 5       # 重试等待（秒）
PROGRESS_FILE = '/tmp/stock_weekly_progress.json'
YEARS_BACK    = 5       # 全量采集年数
...
def upsert_batch(rows: list[dict], table: str = 'stock_weekly') -> int:
    """分批 upsert 到数据库"""
    total = 0
...
def collect_one_stock(ts_code: str, pro, start_date: str, end_date: str, dry_run: bool = False) -> dict:
    ...
    try:
        ...
        if df is None or df.empty:
            return {'code': ts_code, 'rows': 0, 'status': 'empty', 'msg': '无周线数据'}
        ...
        # 写入数据库
        upserted = upsert_batch(rows, 'stock_weekly')
...
# ── 主函数 ────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description='REQ-172: 个股周线数据采集')
    ...
    log.info(f"=== REQ-172: stock_weekly 采集开始 (mode={args.mode}, workers={args.workers}) ===")
